...
# =========================================================
# CENTRAL CONFIG
# ROI 扫描范围 / 基座关节名 / z 等共享量来自 config.py；
# 仅本脚本专用的搜索精度（步长、yaw、seed 等）留在这里。
# =========================================================
CONFIG = {
    # ---------- file ----------
    "urdf_path": os.path.join(_PROJECT_ROOT, _cfg.URDF_RELPATH),
    ...
    "left_base_joint": _cfg.LEFT_BASE_JOINT,
    "right_base_joint": _cfg.RIGHT_BASE_JOINT,
    ...
    "x_min": _cfg.ROI_X_MIN,
    "x_max": _cfg.ROI_X_MAX,

    # 左侧果实带（vehicle +y 一侧）= config.ROI_UPPER 的 y 范围
    "left_side_y_min": _cfg.ROI_UPPER["y_min"],
    "left_side_y_max": _cfg.ROI_UPPER["y_max"],

    # 右侧果实带（vehicle -y 一侧）= config.ROI_LOWER 的 y 范围
    "right_side_y_min": _cfg.ROI_LOWER["y_min"],
    "right_side_y_max": _cfg.ROI_LOWER["y_max"],

    # 这是 vehicle frame 的 z，不是 arm local z（采摘高度切片）
    "z_min": _cfg.Z_DEFAULT,
    "z_max": _cfg.Z_DEFAULT,

    # yaw：先允许一整圈搜索
    "yaw_min": _cfg.YAW_MIN,
    "yaw_max": _cfg.YAW_MAX,
