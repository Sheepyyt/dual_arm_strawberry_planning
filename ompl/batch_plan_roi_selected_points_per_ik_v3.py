#!/usr/bin/env python3
"""
batch_plan_roi_selected_points_per_ik_v3.py

对 roi_table_selected_points.pkl 中的所有点，按“每个 IK 解单独规划”的方式批量运行 OMPL。
输出统一写入：
    ompl/results/batch_plan_roi_per_ik/
...
BATCH_OUT_DIR = os.path.join(SCRIPT_DIR, "results", "batch_plan_roi_per_ik")
...
def main():
    ...
    out_pkl = os.path.join(BATCH_OUT_DIR, "motion_roi_table_selected_points_per_ik.pkl")
    out_csv = os.path.join(BATCH_OUT_DIR, "motion_roi_table_selected_points_per_ik_summary.csv")
    out_txt = os.path.join(BATCH_OUT_DIR, "batch_plan_per_ik_summary.txt")
